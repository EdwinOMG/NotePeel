...
export default function MobileHome({ userEmail, onLogout, onOpenNotebook, onOpenSettings, darkMode }: MobileHomeProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !selectedUploadNotebook) return;
    setUploading(true); setShowQuickUpload(false);
    try {
      let newNote;
      if (files.length === 1) {
        newNote = await notesAPI.upload(files[0], noteType, selectedUploadNotebook);
      } else {
        newNote = await notesAPI.uploadMulti(Array.from(files), noteType, selectedUploadNotebook);
      }
